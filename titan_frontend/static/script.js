...
window.currentThinkingMode = false;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Desabilitar input até a sessão estar pronta
    if (mainInput) {
        mainInput.disabled = true;
        mainInput.placeholder = 'Conectando ao servidor...';
    }
    ...
    try {
        // Inicializar a sessão do usuário primeiro
        await initializeUserSession();
        
        // Habilitar o input após a sessão ser criada com sucesso
        if (mainInput) {
            mainInput.disabled = false;
            mainInput.placeholder = 'Digite sua mensagem ou use / para ver os comandos...';
            mainInput.focus();
        }
        ...
        if (mainInput) {
            mainInput.placeholder = 'Erro de conexão. Tente recarregar a página.';
        }
...
async function sendMessageToServer(message) {
    console.log(' [DEBUG FRONTEND] CSRF Token antes de enviar: ', csrfToken);
...
async function streamWithFetchStream(message, container) {
    console.log('🎯 VERSÃO CORRIGIDA:', message);
    ...
    try {
        ...
        // Limpar thinking
        const result = processThinkingInRealTime(content, container);
        const finalContent = result.afterThinkingContent || content;
        ...
        finalizeStreamingMessage(container, finalContent, result.thinkingContent);
        return finalContent;
...
function finalizeStreamingMessage(container, content, thinking = null) {
    ...
    if (thinking && thinking.trim() && currentThinkingMode) {
        ...
        const contentDiv = container.querySelector('.streaming-content');
        if (contentDiv) {
            // Remover tags <think>...</think> no modo direto
...
function addMessageToChat(content, isUser, systemInfo = null) {
    ...
    if (!isUser) {
        messagesContainer.appendChild(messageDiv);
        setTimeout(() => {
            addMessageActions(messageDiv, content, false);
        }, 50);
    } else {
        messagesContainer.appendChild(messageDiv);
    }
...
async function processStreamResponse(response, container) {
    ...
    try {
        ...
        while (true) {
            ...
            if (done) {
                console.log('✅ [STREAM] Stream completo');
                break;
            }
                ...
                if (line.startsWith('data: ')) {
                    try {
                        ...
                        if (data.type === 'content' && data.content) {
                            ...
                            const contentDiv = container.querySelector('.streaming-content');
                            if (contentDiv) {
                                // Remover thinking tags para exibição
                                const cleanContent = fullContent.replace(/<think>[\s\S]*?<\/think>/g, '').trim();
                                contentDiv.innerHTML = formatMessage(cleanContent);
                            }
                        }
                        
                        if (data.type === 'thinking_done' && data.thinking) {
                            thinkingContent = data.thinking;
                            console.log('🧠 [STREAM] Thinking recebido:', thinkingContent.length, 'chars');
                        }
                        
                        if (data.type === 'error') {
                            if (data.action_required === 'create_account') {
                                showCreateAccountModal(data);
                            } else {
                        ...
                        console.warn('⚠️ [STREAM] Erro no parse:', e);
                    }
                }
            }
        }

        // Fallback se não recebeu 'done'
        const finalContent = fullContent.replace(/<think>[\s\S]*?<\/think>/g, '').trim();
        finalizeStreamingMessage(container, finalContent, thinkingContent);
        
    } catch (error) {
        console.error('❌ [STREAM] Erro no processamento:', error);
        
        // Mostrar erro na interface
        const contentDiv = container.querySelector('.streaming-content');
        if (contentDiv) {
            contentDiv.innerHTML = 'Erro na comunicação com o servidor';
        }
...
async function sendMessageToServer(message) {
    ...
    try {
        console.log('📡 [SEND] Enviando para API:', finalMessage);
        ...
        // ✅ PROCESSAR STREAM COM FUNÇÃO CORRIGIDA
        await processStreamResponse(response, streamContainer);
        console.log('✅ [SEND] Stream processado com sucesso');
...
setInterval(() => {
    if (typeof window.streamWithFetchStream === 'undefined' || window.streamWithFetchStream.toString().includes('VERSÃO CORRIGIDA') === false) {
        window.streamWithFetchStream = async function(message, container) {
            const response = await fetch('https://api.outzapp.com/chat-stream', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ mensagem: message, thinking_mode: false })
            });
            const text = await response.text();
            let content = '';
            text.split('\n').forEach(line => {
                if (line.startsWith('data: ')) {
                    try {
                        const data = JSON.parse(line.slice(6));
                        if (data.type === 'content' && data.content) content += data.content;
                    } catch (e) {}
                }
            });
            const clean = content.replace(/<think>[\s\S]*?<\/think>/g, '').trim();
            const div = container.querySelector('.streaming-content');
            if (div) div.textContent = clean;
            return clean;
        };
        window.getHeaders = () => ({ 'Content-Type': 'application/json' });
    }
}, 3000);
